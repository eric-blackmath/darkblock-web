...
export default function FileChooser({ fileSelectionHandler, level }) {
    ...
    function ekUploadtwo() {
      ...
      function parseFile(file) {
        ...
        if (isGood) {
          ...
          document.getElementById("file-imagetwo").src = URL.createObjectURL(file);
        } else {
      ...
      function setProgressMaxValue(e) {
        var pBar = document.getElementById("file-progresstwo");
      ...
      function uploadFile(file) {
        var xhr = new XMLHttpRequest(),
          fileInput = document.getElementById("class-roster-file"),
          pBar = document.getElementById("file-progresstwo"),
          fileSizeLimit = 1024; // In MB
        if (xhr.upload) {
          // Check if file is less than x MB
          if (file.size <= fileSizeLimit * 1024 * 1024) {
            ...
            xhr.open(
              "POST",
              document.getElementById("file-upload-formtwo").action,
              true
            );
            xhr.setRequestHeader("X-File-Name", file.name);
            xhr.setRequestHeader("X-File-Size", file.size);
